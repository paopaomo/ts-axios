...
describe('requests', () => {
  ...
  test('should return JSON when resolved', done => {
    ...
    getAjaxRequest().then(request => {
      request.respondWith({ status: 200, statusText: 'OK', responseText: '{"error": 0}' });
  ...
  test('should return JSON when rejecting', done => {
    ...
    getAjaxRequest().then(request => {
      ...
      setTimeout(() => {
        ...
        expect(res.data.error).toBe('BAD USERNAME');
  ...
  test('should support correct response', done => {
    let res: AxiosResponse;

    axios.post('/foo').then(response => {
      res = response;
    });
